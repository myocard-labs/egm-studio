...
### Block 13 — User documentation ✓ SHIPPED

> **Shipped 2026-07-07.** `docs/usage.md` — the full user manual: install + first
> launch, the shell, all four modes with whole-window screenshots + walkthroughs, a
> per-recipe figure-reading guide with good/bad example renders, the save /
> Phase-tree flow, the headless `egm-studio-render` CLI reference, navigation, and
> troubleshooting. Screenshots + example figures are generated by committed scripts
> (`scripts/capture_screenshots.py`, `scripts/render_figure_examples.py`) and cached
> under `docs/screenshots/`; added the first `CHANGELOG.md`. Deferred: a curated
> keyboard-shortcut set (none defined in v0.1) and any `docs/walkthroughs/` split.
