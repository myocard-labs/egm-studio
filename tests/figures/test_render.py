...
def _stub_recipe(spec: FigureSpec) -> Figure:
...
def registered_stub() -> Iterator[str]:
    ...
    global registry stays empty for other tests."""
...
def test_unknown_recipe_raises() -> None:
    """render() raises UnknownRecipeError naming the offending recipe; at Block 2
    the registry is empty, so the message explicitly says no recipes are
    registered yet."""
    with pytest.raises(UnknownRecipeError) as excinfo:
        render(_spec(recipe="definitely-not-a-recipe"))
    assert excinfo.value.recipe == "definitely-not-a-recipe"
    assert "no recipes are registered" in str(excinfo.value)


def test_dispatches_to_registered_recipe(tmp_path: Path, registered_stub: str) -> None:
    """A registered recipe is invoked and its Figure written to the spec's
    output path; render returns that path and the file exists."""
    out = tmp_path / "fig.png"
    result = render(_spec(recipe=registered_stub, path=str(out)))
    assert result == out
    assert out.exists()
...
def test_output_path_override(tmp_path: Path, registered_stub: str) -> None:
    ...
    result = render(_spec(recipe=registered_stub, path="ignored.png"), output_path=out)
    assert result == out
    assert out.exists()
...
def test_render_creates_missing_parent_dirs(tmp_path: Path, registered_stub: str) -> None:
    ...
    render(_spec(recipe=registered_stub, path=str(out)))
    assert out.exists()
