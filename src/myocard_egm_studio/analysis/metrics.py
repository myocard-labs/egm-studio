"""ROC / AUROC metrics for the synthetic-val classifier figures.

Pure numpy / scipy, like :mod:`.distributions` — the math behind
``roc-curve-multi-line`` (and the calibration / confusion recipes to come). No
sklearn: ROC is a sort over the predicted probabilities and AUROC is a rank
statistic, both small enough to own here, which keeps the dependency surface
lean and matches the analysis layer's numpy/scipy-only rule.
